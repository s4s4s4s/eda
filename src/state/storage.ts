...
/** Пишет состояние в localStorage. Ошибка записи (квота, приватный режим)
    молча проглатывается — потерять возможность сохранить хуже, чем уронить
    экран, но и падать здесь не на чем настаивать. */
...
export function saveState(state: AppState): SaveResult {
  ...
  try {
    ...
    return {
      ok: false,
      error: quota
        ? 'Хранилище браузера переполнено — запись не сохранена. Выгрузите старые дни в CSV и очистите дневник.'
